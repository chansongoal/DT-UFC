...
class StableDiffusion3Pipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin, SD3IPAdapterMixin):
    ...
    def generate_image_from_latents(self, latents, output_type="pil"):
        ...
        Args:
            latents (`torch.FloatTensor`): Precomputed latents.
            output_type (`str`): The desired output format of the image ("pt", "pil" or "numpy").

        Returns:
            `PIL.Image.Image` or `np.ndarray`: The generated image in the specified format.
        """
        # print(self.vae.config.scaling_factor, self.vae.config.shift_factor)
        latents = (latents / self.vae.config.scaling_factor) + self.vae.config.shift_factor
