    ...
    def forward(
        ...
        for decoder_layer in self.layers:
            ...
            if layer_idx == 32:
                ...
                # only truncation
                # hidden_states = torch.clip(layer_outputs[0], min=-5, max=5)

                # only quantization
                # bit_depth = 8
                # min_v, max_v = layer_outputs[0].min(), layer_outputs[0].max()
                # scale = (2**bit_depth -1) / (max_v - min_v)
                # quant_feat = ((layer_outputs[0]-min_v) * scale).round().clamp(0,(2**bit_depth-1))
