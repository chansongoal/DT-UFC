...
def generate_cross_eval_commands(data_root_train, data_root_test, model_type_train, model_type_test, task_train, task_test, 
                                 trun_flag, trun_high, trun_low, quant_type, samples, bit_depth, quant_points_name, 
    ...
    eval_command = (
        ...
        f"-d {data_root_test}/{model_type_test}/{task_test}/{model_name}/decoded/trained_{task_train}_trunl{trun_low}_trunh{trun_high}_{quant_type}{samples}_bitdepth{bit_depth}/lambda{lambda_value}_epochs{epochs}_lr{learning_rate}_bs{batch_size}_patch{patch_size.replace(' ', '-')} "
        # load specified checkpoint accordingly, use the epoch information
        f"--per-image -p {data_root_train}/{model_type_train}/{task_train}/{model_name}/training_models/trunl{trun_low}_trunh{trun_high}_{quant_type}{samples}_bitdepth{bit_depth}/"
        ...
        f"--model_type={model_type_test} --task={task_test} --trun_flag={trun_flag} --trun_low={trun_low} --trun_high={trun_high} --quant_type={quant_type} --qsamples={samples} --bit_depth={bit_depth} --quant_points_name={quant_points_name} "
        f">{data_root_test}/{model_type_test}/{task_test}/{model_name}/encoding_log/trained_{task_train}_trunl{trun_low}_trunh{trun_high}_{quant_type}{samples}_bitdepth{bit_depth}/"
